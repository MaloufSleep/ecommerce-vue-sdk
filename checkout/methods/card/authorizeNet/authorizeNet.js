...
export default class AuthorizeNet {
    ...
    constructor(merchantId, publicKey){
    ...
    static fromPaymentService(service){
        ...
        return new AuthorizeNet(merchantId, publicKey)
    }

    loadScript(){
        if(!isClient() || window.Accept) return Promise.resolve(true)
        const src = process.env.PAYMENT_ENV === 'production' ? 'https://js.authorize.net/v1/Accept.js' : 'https://jstest.authorize.net/v1/Accept.js'
