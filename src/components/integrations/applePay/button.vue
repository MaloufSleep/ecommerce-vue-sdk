<template>
    <button v-if="available" :class="buttonClasses" @click="onClick">
            ...
            window.ApplePaySession.canMakePaymentsWithActiveCard(this.merchantId).then(canMakePayments => {
                if(canMakePayments){
                    ...
                    if(window.ApplePaySession){
                        this.setup = this.available = true
                    }
                }
            })
        },
        openPaymentSetup(){
            window.ApplePaySession.openPaymentSetup(this.merchantId).then(success => {
                if(!success){
                    this.$emit('openPaymentSetupFailed')
                    console.error('Error: ApplePaySession.openPaymentSetup() failed')
                }
            }).catch(err => {
                console.error(err)
            })
